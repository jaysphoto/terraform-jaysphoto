...
data "aws_iam_policy_document" "grant_organization_admin_access" {
  count = var.organizational_unit.admin_role_name == "" ? 0 : 1

  statement {
    actions = ["sts:AssumeRole"]
    effect = "Allow"
    resources =  flatten([for org in aws_organizations_account.ou_account : "arn:aws:iam::${org.id}:role/OrganizationAccountAccessRole" ])
